...
function AdminDashboard({ tasks, projects, healthCenters }: {
  tasks: Task[];
  projects: Project[];
  healthCenters: HealthCenter[];
}) {
  ...
  const hcRows = useMemo(() => {
    return healthCenters
      .map((hc) => {
        ...
        const hcOverdue = hcOpen.filter((t) => {
          const d = parseTaskDate(t.dueDate);
          return d !== null && isBefore(d, today);
        });
  ...
  return (
    <div className="px-[24px] py-8 max-w-[1200px] mx-auto space-y-8">
      {/* Greeting */}
      <div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {hcRows.map(({ hc, openCount, overdueCount }) => (
                <tr
                  key={hc.name}
                  className="border-b border-[#e4e4e7] last:border-0 hover:bg-[#f5f5f5] transition-colors"
                >
                  ...
                  <td className="px-4 py-3">
                    <ComplianceDot value={hc.compliance.fqhc} />
                  </td>
                  <td className="px-4 py-3">
                    <ComplianceDot value={hc.compliance.ftca} />
                  </td>
